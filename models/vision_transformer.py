...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, cls_only=True):
        ...
        for end_idx in idx_crops:
            _out = self.forward_features(torch.cat(x[start_idx: end_idx]), cls_only)
            if start_idx == 0:
                output = _out
            else:
...
def deit_tiny(patch_size=16, img_size=[224], **kwargs):
...
def deit_small(patch_size=16, img_size=[224], **kwargs):
    model = VisionTransformer(
        img_size=img_size,
